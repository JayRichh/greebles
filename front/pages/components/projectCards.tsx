      ...
      ? projectData.filter((project) =>
          selectedTech.every((tech) => {
            if (universalBadgeMapping[tech as keyof typeof universalBadgeMapping]) {
              return (universalBadgeMapping[tech as keyof typeof universalBadgeMapping] as string[]).some((mappedTech) =>
                project.details.technologies.includes(mappedTech)
              );
            } else {
        ...
        <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4rem', marginTop: '2rem' }}>
            ...
            <div style={{ width: '50%' }}>
              ...
              {project.details.technologies.slice(3).filter((tech) => !project.details.technologies.slice(0, 3).includes(tech)).map((tech) => {
                const techInfo = techIcons[tech];
                if (!techInfo) {
                  console.warn(`No specific icon found for technology: ${tech}. Using default icon.`);
                  return (
                    <Badge
                      ...
                      style={{
                        ...
                        backgroundColor: '#f0f0f0',
                        color: '#333',
                        transition: 'filter 0.2s cubic-bezier(0.4, 0, 0.2, 1), box-shadow 0.2s cubic-bezier(0.4, 0, 0.2, 1)',
                      }}
                      leftSection={<DiTerminal size={14} />}
                    >
                      {tech}
