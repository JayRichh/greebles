/**
 * Manages the WebSocket connection to the server and handles incoming and outgoing messages.
 *
 * The `WebSocketManager` class is responsible for establishing and maintaining the WebSocket connection to the server.
 * It provides methods for sending client messages and registering handlers for incoming server messages.
 *
 * The class uses the `msgpackr` library for efficient message packing and unpacking.
 *
 * @param game - The `Game` instance that the `WebSocketManager` is associated with.
 */
...
export class WebSocketManager {
	...
	constructor(game: Game) {
		...
		this.serverUrl = isProduction ? 'wss://evalugem.com/ws' : 'ws://localhost:8001'
			...
			if (!this.isConnected()) {
				...
				this.websocket.addEventListener('close', (closeEvent) => {
					if (closeEvent.wasClean) {
						console.log(`WebSocket connection closed cleanly, code=${closeEvent.code}, reason=${closeEvent.reason}`)
					} else {
