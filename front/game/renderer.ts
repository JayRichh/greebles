...
export class Renderer extends THREE.WebGLRenderer {
	...
	public update(entities: Entity[]) {
		...
		if (followedEntity && this.directionalLight) {
			...
			if (position) {
				this.directionalLight.position.lerp(new THREE.Vector3(position.x, position.y + 150, position.z - 15), 0.1)
				this.directionalLight.target.position.lerp(new THREE.Vector3(position.x, position.y, position.z), 0.1)
			}
		}
		this.camera.update()
